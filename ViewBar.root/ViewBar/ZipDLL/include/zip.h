...
#ifdef DLLEXPORT 
#else 
#define DLLEXPORT _declspec(dllimport) // 注意这里是dllimport
#endif 


DLLEXPORT HZIP CreateZip(const TCHAR *fn, const char *password);
DLLEXPORT HZIP CreateZip(void *buf,unsigned int len, const char *password);
DLLEXPORT HZIP CreateZipHandle(HANDLE h, const char *password);
...
DLLEXPORT ZRESULT ZipAdd(HZIP hz,const TCHAR *dstzn, const TCHAR *fn);
DLLEXPORT ZRESULT ZipAdd(HZIP hz,const TCHAR *dstzn, void *src,unsigned int len);
DLLEXPORT ZRESULT ZipAddHandle(HZIP hz,const TCHAR *dstzn, HANDLE h);
DLLEXPORT ZRESULT ZipAddHandle(HZIP hz,const TCHAR *dstzn, HANDLE h, unsigned int len);
DLLEXPORT ZRESULT ZipAddFolder(HZIP hz,const TCHAR *dstzn);
...
DLLEXPORT ZRESULT ZipGetMemory(HZIP hz, void **buf, unsigned long *len);
...
DLLEXPORT ZRESULT CloseZip(HZIP hz);
// CloseZip - the zip handle must be closed with this function.

DLLEXPORT unsigned int FormatZipMessage(ZRESULT code, TCHAR *buf,unsigned int len);
...
DLLEXPORT ZRESULT CloseZipZ(HZIP hz);
DLLEXPORT unsigned int FormatZipMessageZ(ZRESULT code, char *buf,unsigned int len);
DLLEXPORT bool IsZipHandleZ(HZIP hz);
#ifdef _unzip_H
#undef CloseZip
