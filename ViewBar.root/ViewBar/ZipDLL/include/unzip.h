...
DLLEXPORT HZIP OpenZip(const TCHAR *fn, const char *password);
DLLEXPORT HZIP OpenZip(void *z,unsigned int len, const char *password);
DLLEXPORT HZIP OpenZipHandle(HANDLE h, const char *password);
...
DLLEXPORT ZRESULT GetZipItem(HZIP hz, int index, ZIPENTRY *ze);
...
DLLEXPORT ZRESULT FindZipItem(HZIP hz, const TCHAR *name, bool ic, int *index, ZIPENTRY *ze);
// FindZipItem - finds an item by name. ic means 'insensitive to case'.
// It returns the index of the item, and returns information about it.
// If nothing was found, then index is set to -1 and the function returns
// an error code.

DLLEXPORT ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
DLLEXPORT ZRESULT UnzipItem(HZIP hz, int index, void *z,unsigned int len);
DLLEXPORT ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
...
DLLEXPORT ZRESULT SetUnzipBaseDir(HZIP hz, const TCHAR *dir);
...
DLLEXPORT ZRESULT CloseZipU(HZIP hz);
DLLEXPORT unsigned int FormatZipMessageU(ZRESULT code, TCHAR *buf,unsigned int len);
DLLEXPORT bool IsZipHandleU(HZIP hz);
#ifdef _zip_H
#undef CloseZip
