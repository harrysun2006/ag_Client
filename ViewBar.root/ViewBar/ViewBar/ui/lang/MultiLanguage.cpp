...
// load the resource dll directly
HINSTANCE CMultiLanguage::LoadResourceDLL( LPCTSTR lpszDllNamePath)
{
	HINSTANCE hLangDLL = NULL;

	hLangDLL = ::LoadLibrary(lpszDllNamePath);
	if(hLangDLL != NULL)
		return hLangDLL;
...
HINSTANCE CMultiLanguage::LoadLangResourceDLL(LPCTSTR szModuleName, LANGID langUpdateId)
{
	...
	for ( iLocale = 0; iLocale < m_nLocales; iLocale++ )
	{
