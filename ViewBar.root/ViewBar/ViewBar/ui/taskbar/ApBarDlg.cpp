...
void CAppBarDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{
		/*
		 * prevent appbar from docking to the left and right
		 *
...
// set the pos of docking
void CAppBarDlg::SetDockingSide(UINT uSide)
{
	m_pAppBar->SetSide(uSide);
	m_iPosition = uSide;
}

LRESULT CAppBarDlg::AppBarCallback(WPARAM wParam, LPARAM lParam)
{
	if (wParam != ABN_POSCHANGED)
		return 0;

	// get the new position of ViewBar
	thePrefs.SetDockTop( GetViewBarPos() == ABE_TOP ? true : false);

	// if ViewBar and TaskBar are on the same side, just return
	if (IsTheSameSide() == FALSE)
		return 0;

	// dock ViewBar
	if (thePrefs.IsDockTop())
	{
		this->SetDockingSide(ABE_TOP);
	}
	else
	{
		this->SetDockingSide(ABE_BOTTOM);
	}

	return 0;
}

UINT CAppBarDlg::GetViewBarPos()
{
	CRect desktop;
	CRect viewbar;
...
BOOL CAppBarDlg::IsTheSameSide()
{
	bool bViewBarOnTop = thePrefs.IsDockTop();

	int y = GetSystemMetrics(SM_CYSCREEN);

	CWnd   *pTaskBar=FindWindow(_T("Shell_TrayWnd"), NULL);   
	if(pTaskBar == NULL)
		return FALSE;
	...
	bool bTaskBarOnTop = false;

	if (rctTaskBar.top > y/2)
		bTaskBarOnTop = false;
	else
		bTaskBarOnTop = true;

	if (bTaskBarOnTop == bViewBarOnTop)
	{
		return TRUE;
	}
	else
