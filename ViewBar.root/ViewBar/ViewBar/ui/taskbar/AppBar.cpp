...
void CAppBar::Size()
{
	...
	if (m_bAppRegistered)
	{
		...
		//TRACE(_T("^^^^^^^^^QuerySetPos^rc = %d, %d, %d, %d\n"),
...
void CAppBar::QuerySetPos(UINT uEdge, LPRECT lprc, PAPPBARDATA pabd, BOOL bMove)
{
	...
	if (bMove)
	{
		...
		TRACE(_T("^^^^^^^^^QuerySetPos^rc = %d, %d, %d, %d\n")
			, pabd->rc.left, pabd->rc.top, pabd->rc.right, pabd->rc.bottom);
...
BOOL CAppBar::NoAutoHide()
{
	...
	{
        strMsg.Format (_T("%s"), STR_ERROR_NOT_HIDDEN);
		TRACE (strMsg);
		return FALSE;
	}
	...
	if (!fSuccess)
	{
        strMsg.Format (_T("%s"), STR_ERROR_SET_AUTOHIDE);
		TRACE (strMsg);
        return FALSE;
	}
	else
