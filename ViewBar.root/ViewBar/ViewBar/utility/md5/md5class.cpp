...
namespace utility
{
	...
	*/
	...
	void CMD5::GetMD5(const CString &in, CString &out)
	{
		CMD5 x;
		//const char *pIn = CMyUtility::GetAnsiString(in);
		//x.setPlainText(pIn);
		CStringA mbIn = CMyUtility::GetCStringA(in);
		x.setPlainText(mbIn.GetBuffer(NULL));
		const char *pOut = x.getMD5Digest();
		TCHAR *pOutW = CMyUtility::GetUnicodeString(pOut, 33);
		out.Format(_T("%s"), pOutW);
		delete [] pOutW;
		//delete [] pIn;
	}
